...
/** Igual a `ParsedEmbroideryOrderError["duplicate"]` — conserva el mensaje del
...
/**
 * Estado del formulario de alta de orden de bordado.
 *
 ...
 * valor almacenado, así que preseleccionarlo muestra la verdad en vez de
 * fingir que "sin elegir" es un estado posible. Ojo con la consecuencia: bajo
 * la convención de etiquetas del proyecto ese `1` se lee "Alta", de modo que
 * el default efectivo de TODA orden de bordado es prioridad Alta — es una
 * herencia del backend, no una decisión de esta pantalla.
 */
export function useEmbroideryOrderForm({ onSuccess }: { onSuccess?: () => void } = {}) {
  ...
  // excluyente con `serverBanner`: cuando está presente, el formulario
  // muestra el bloque informativo de duplicado EN VEZ DEL banner rosa de
  // validación — son el mismo rechazo, no dos avisos a la vez.
  ...
  /**
   * Reparte el error ya normalizado: los de campo (forma C) bajo su input,
   * el duplicado (forma D) a su propio bloque informativo, y todo lo demás al
  ...
  const handleServerError = (parsed: ParsedEmbroideryOrderError) => {
    ...
    setServerBanner(`No se creó la orden de bordado. ${detail}`);
  };
  ...
  // ─── Formulario ───────────────────────────────────────────────────────────
  const defaultValues = useMemo<CreateEmbroideryOrderFormValues>(
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      const parsed = CreateEmbroideryOrderFormSchema.safeParse(value);
      if (!parsed.success) {
      ...
      try {
        await createOrder(buildEmbroideryOrderPayload(parsed.data));
