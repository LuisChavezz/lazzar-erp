...
import {
  EmbroideryOrderDetallesOverrideSchema,
  type CreateEmbroideryOrderFormValues,
} from "../schemas/embroidery-order.schema";
import type {
  EmbroideryOnboardingDetalle,
  EmbroideryOrderDetalleOverride,
} from "../interfaces/embroidery.interface";
import { buildEmbroideryOrderPayload } from "../utils/buildEmbroideryOrderPayload";
import { useEmbroideryOnboarding } from "./useEmbroideryOnboarding";
import {
  useCreateEmbroideryOrder,
  type ParsedEmbroideryOrderError,
} from "./useCreateEmbroideryOrder";

/** Igual a `ParsedEmbroideryOrderError["duplicate"]` — ver `useEmbroideryStep2Form`. */
type EmbroideryDuplicateState = NonNullable<ParsedEmbroideryOrderError["duplicate"]>;

interface UseEmbroideryStep2FormParams {
  header: CreateEmbroideryOrderFormValues;
  onSuccess: () => void;
}

/** Nombre legible de la línea. `producto_nombre` es nullable en el contrato. */
export const embroideryLineProductoNombre = (
  row: EmbroideryOnboardingDetalle,
 ...
 */
const ceilingOf = (row: EmbroideryOnboardingDetalle): number =>
 ...
 * pero su saneador interno deja un punto colgante ("1." al teclear "1.5"), y
 * ese valor a medias no debe llegar ni a la pantalla ni al payload. La
 * validación de Zod al enviar (`.int()`) queda como segunda red, no como
 * primera.
 */
...
/**
 * Paso 2 del alta de orden de bordado: qué líneas del pedido entran en esta
 * orden y con cuántas piezas.
 *
 * ── Dónde vive el estado ────────────────────────────────────────────────────
 * El ENCABEZADO vive en `EmbroideryOrderStepManager` (por eso "Regresar" lo
 * conserva); la SELECCIÓN POR LÍNEA vive aquí, en el hook del paso, que solo
 * está montado mientras el Paso 2 está en pantalla. Es exactamente el reparto
 * de `PickingStepManager`/`usePickingStep2Form`, y tiene una consecuencia
 * deliberada: regresar al Paso 1 descarta la selección, y volver a entrar la
 * vuelve a sembrar desde datos RECIÉN traídos del servidor. Con ello, cambiar
 * de pedido en el Paso 1 no puede arrastrar líneas del pedido anterior —no hay
 * estado que limpiar, porque no sobrevive—.
 *
 * ── Frescura de los saldos ──────────────────────────────────────────────────
 * Se consume el MISMO `useEmbroideryOnboarding` del Paso 1 (misma llave de
 * caché): el catálogo ya trae `detalles` por pedido, así que no hay una
 * segunda petición con alcance al padre como en picking. Ese hook fuerza
 * `staleTime: 0` + `refetchOnMount: "always"` (ver Fase 1), de modo que entrar
 * al Paso 2 recarga los saldos antes de proponer cantidades.
 *
 * ── Techo por línea ─────────────────────────────────────────────────────────
 * `cantidad_pendiente` NO se valida en el esquema de Zod: es dato del servidor
 * que puede bajar entre la carga y el envío (otra OB del mismo pedido lo
 * consume). Se aplica en tres momentos, como en picking: al capturar (el input
 * clampa), al reconciliar (tras un refetch, `useEffect` de abajo) y al enviar
 * (corte local antes del POST). El backend sigue siendo la fuente de la
 * verdad: pasar los tres no garantiza que acepte.
 */
export function useEmbroideryStep2Form({
  header,
  onSuccess,
}: UseEmbroideryStep2FormParams) {
   ...
   * describen la misma entidad y tenerlas con el mismo tipo de llave elimina la
   * docena de `String(id)` que hacían falta para cruzarlas, y con ellos la
   * posibilidad de que una consulta use el tipo equivocado y falle en silencio.
   *
   ...
   * `useMemo`: solo alimenta el render, así que el React Compiler lo memoiza
   * por su cuenta (ver CLAUDE.md, "React Compiler").
   */
   ...
   * refetch fallido con datos en caché conserva la tabla y lo capturado, y
   * avisa por toast desde `useEmbroideryOnboarding` — crítico aquí, porque el
   * 400 de exceso dispara un refetch con el usuario a media captura y un fallo
   * de red en ese momento borraba todas las cantidades. Mismo criterio que
   * `EmbroideryView`.
   */
   ...
   * tratamiento visual: el `detalles_exceso` del backend (strings ya
   * formateados, que NO se reinterpretan) y el corte local previo al envío.
   */
  const [issueLines, setIssueLines] = useState<string[]>([]);
  const [staleNotice, setStaleNotice] = useState<string | null>(null);
  const [duplicate, setDuplicate] = useState<EmbroideryDuplicateState | null>(null);
   ...
   * datos: un refetch posterior —el que dispara el 400 de exceso— no debe
   * volver a marcar lo que el usuario ya había desmarcado. Ajustar los valores
   * a los saldos nuevos es trabajo del efecto de reconciliación de abajo.
   */
  ...
  /**
   * Reconciliación contra los saldos recién llegados: re-clampa lo capturado
   * al techo ACTUAL y desmarca lo que se quedó sin pendiente. Es lo que hace
   * que, tras el 400 de exceso (que recarga el onboarding), la pantalla muestre
   * los pendientes nuevos en vez de los que el usuario ya intentó enviar — lo
   * MOSTRADO nunca puede divergir de lo que se enviaría. Mismo efecto que la
   * reconciliación de `usePickingStep2Form`.
   */
  useEffect(() => {
    setQuantities((prev) => {
      let changed = false;
   ...
   * propósito —antes sobrevivía a la corrección que lo volvía obsoleto y se
   * quedaba en pantalla asegurando algo que ya no era cierto—.
   */
  const clearIssues = () => {
  ...
  const handleServerError = (parsed: ParsedEmbroideryOrderError) => {
    ...
    if (parsed.duplicate) {
      ...
      // `detalles_override`, y este paso lo manda siempre. Se conserva porque
      // el que decide es el backend, no esta pantalla: si algún día vuelve a
    ...
    setServerBanner(
      fieldMessages.length > 0
        ...
        : parsed.formError ?? parsed.messages[0] ?? "No se pudo crear la orden de bordado.",
    );
    ...
    if (parsed.excessLines) {
      ...
      // `["embroidery-onboarding"]` (Fase 1); el `refetch` explícito garantiza
      // que la recarga ocurra aunque la invalidación no encuentre un observador
      // activo, y el efecto de reconciliación de arriba baja las cantidades
      // capturadas a los pendientes nuevos.
      //
      ...
      // backend, y afirmar "se actualizaron las cantidades" mientras los
      // inputs siguen mostrando lo que el servidor acaba de rechazar invitaba a
      // reenviar exactamente el mismo payload.
      setStaleNotice(
        "Los pendientes de este pedido cambiaron (otra orden de bordado los consumió). Se están actualizando las cantidades; revísalas antes de volver a intentar.",
      );
      void refetch();
    }
  };
  ...
  const { mutateAsync: createOrder, isPending } =
    useCreateEmbroideryOrder(handleServerError);
  ...
  const handleSubmit = async () => {
    ...
    const lines: EmbroideryOrderDetalleOverride[] = [];
    ...
    for (const row of rows) {
      ...
      const nombre = embroideryLineProductoNombre(row);
    ...
    // Segunda red, ya sobre el payload exacto que viajaría: entero positivo,
    // sin ids repetidos y no vacío. Replica lo que el serializer rechazaría con
    // un 400 bajo la clave `detalles_override`.
    const parsed = EmbroideryOrderDetallesOverrideSchema.safeParse(lines);
    if (!parsed.success) {
      setServerBanner(parsed.error.issues[0]?.message ?? "Revisa las cantidades capturadas.");
      return;
    }
    ...
    try {
      await createOrder(buildEmbroideryOrderPayload(header, parsed.data));
      onSuccess();
    } catch {
