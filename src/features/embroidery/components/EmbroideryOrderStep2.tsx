...
import { useEmbroideryStep2Form } from "../hooks/useEmbroideryStep2Form";
import type { CreateEmbroideryOrderFormValues } from "../schemas/embroidery-order.schema";
import { EmbroideryOrderLinesTable } from "./EmbroideryOrderLinesTable";

interface EmbroideryOrderStep2Props {
  header: CreateEmbroideryOrderFormValues;
  onBack: () => void;
  onSuccess: () => void;
  /**
   * Abre el diálogo de detalle (`EmbroideryOrderDetailDialog`, montado en
   * `EmbroideryView`) para el `id` dado. Lo usa el bloque de duplicado (409)
   * para llevar a la orden YA EXISTENTE — no navega a ninguna ruta, es la misma
   * mecánica de estado (`openOrderId`) que abre el detalle desde el listado.
   */
  onViewExistingOrder: (id: number) => void;
}

/**
 * Paso 2 del alta de orden de bordado: qué prendas del pedido entran en esta
 ...
 * que es el que de verdad explica qué pasa. Ver `buildEmbroideryOrderPayload`.
 */
export function EmbroideryOrderStep2({
  header,
  onBack,
  onSuccess,
  onViewExistingOrder,
}: EmbroideryOrderStep2Props) {
  const {
    pedido,
    rows,
    ceilings,
    ...
    isPending,
    handleSubmit,
  } = useEmbroideryStep2Form({ header, onSuccess });
  ...
  if (isLoading) {
    return (
      <Loader
        ...
        message="Consultando qué prendas del pedido siguen pendientes de bordar..."
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      {/* ── Bloque de duplicado (409) ─────────────────────────────────────
          ...
          `useEmbroideryStep2Form`—, nunca se muestran ambos a la vez. */}
      {duplicate && (
        <div
          role="alert"
          ...
          <div className="rounded-2xl border border-amber-200 dark:border-amber-800/60 bg-amber-50 dark:bg-amber-900/20 p-6 flex items-start gap-4">
            ...
            <div>
              <h3 className="text-sm font-semibold text-amber-800 dark:text-amber-200">
                {rows.length === 0
                  ? "Este pedido ya no tiene prendas por bordar"
                  : "Todas las prendas de este pedido ya están programadas"}
              </h3>
              <p className="text-xs text-amber-700 dark:text-amber-300 mt-1">
                Otra orden de bordado pudo haberlas cubierto mientras este formulario
                estaba abierto. Regresa para elegir otro pedido.
              </p>
            </div>
          </div>
        ) : (
          <EmbroideryOrderLinesTable
            rows={rows}
            ceilings={ceilings}
      ...
      {/* ── Aviso de permanencia ─────────────────────────────────────────
          Sigue siendo cierto que la orden nace en el primer estatus del enum
          —"Sin trabajar", el default del backend— y consume folio.
      ...
      >
        ...
        <p className="min-w-0 flex-1 text-xs text-amber-700 dark:text-amber-300">
          La orden se crea con estatus <strong>Sin trabajar</strong> y consume un folio de la
