...
import { updateCompany } from "../services/actions";
...
export const useUpdateCompany = (setError?: UseFormSetError<CompanyFormValues>) => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationKey: ["update-company"],
    mutationFn: ({ id, values }: { id: number; values: CompanyFormValues }) => updateCompany(id, values),
