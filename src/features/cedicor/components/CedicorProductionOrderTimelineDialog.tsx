...
import type { ProductionOrder, ProductionOrderStatus, ProductionOrderEventRecord } from "../interfaces/cedicor-production-order.interface";
import { PRODUCTION_ORDER_STEPS, PRODUCTION_ORDER_STATUS_LABELS } from "../interfaces/cedicor-production-order.interface";

// ── Configuración visual por estatus del flujo ────────────────────────────────

const ESTATUS_CFG: Record<ProductionOrderStatus, { badgeCls: string; dotCls: string }> = {
  orden_generada:          { badgeCls: 'bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400',           dotCls: 'bg-blue-500' },
  op_enviada_correo:       { badgeCls: 'bg-indigo-50 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-400',   dotCls: 'bg-indigo-500' },
  ficha_tecnica_generada:  { badgeCls: 'bg-violet-50 text-violet-700 dark:bg-violet-500/10 dark:text-violet-400',   dotCls: 'bg-violet-500' },
  verificacion_materiales: { badgeCls: 'bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400',       dotCls: 'bg-amber-500' },
  entregado_a_trazo:       { badgeCls: 'bg-teal-50 text-teal-700 dark:bg-teal-500/10 dark:text-teal-400',           dotCls: 'bg-teal-500' },
  trazo_a_corte:           { badgeCls: 'bg-cyan-50 text-cyan-700 dark:bg-cyan-500/10 dark:text-cyan-400',           dotCls: 'bg-cyan-500' },
  corte_completado:        { badgeCls: 'bg-orange-50 text-orange-700 dark:bg-orange-500/10 dark:text-orange-400',   dotCls: 'bg-orange-500' },
  consumo_capturado:       { badgeCls: 'bg-sky-50 text-sky-700 dark:bg-sky-500/10 dark:text-sky-400',               dotCls: 'bg-sky-500' },
...
interface TimelineStepPO {
  ...
  esCurrent: boolean;
  /** Datos del evento real si existe en historial */
  evento?: ProductionOrderEventRecord;
}

/**
 * Construye la lista de pasos del timeline combinando los 9 pasos
 * canónicos con los eventos reales del historial de la orden.
 */
function buildTimelineSteps(order: ProductionOrder): TimelineStepPO[] {
  const { historial, paso_actual, estatus } = order;
  const esCancelado = estatus === 'cancelado';
  const esBloqueado = estatus === 'material_faltante';

  return PRODUCTION_ORDER_STEPS.map((stepKey, idx) => {
    ...
    // Para pasos completados se usa el estatus canónico del flujo;
    // para el paso actual se conserva el estatus real (cancelado/bloqueado)
...
interface TimelineItemPOProps {
  step: TimelineStepPO;
  isLast: boolean;
}

function TimelineItemPO({ step, isLast }: TimelineItemPOProps) {
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              >
                ...
                {PRODUCTION_ORDER_STATUS_LABELS[step.estatus]}
              </span>
            </div>
            ...
            {/* Para pasos futuros se muestra texto apagado */}
...
interface CedicorProductionOrderTimelineDialogProps {
  order: ProductionOrder;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function CedicorProductionOrderTimelineDialog({
  order,
  open,
  onOpenChange,
}: CedicorProductionOrderTimelineDialogProps) {
  ...
  return (
    ...
    >
      <div className="space-y-4 mt-1">
        ...
        <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-white/3 border border-slate-100 dark:border-white/5">
          ...
          <div className="flex-1 min-w-0">
            ...
            >
              ...
              {PRODUCTION_ORDER_STATUS_LABELS[order.estatus]}
            </span>
          </div>
          <div className="text-right shrink-0">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="p-3 rounded-xl bg-slate-50 dark:bg-white/3 border border-slate-100 dark:border-white/5 text-center">
            ...
            <p className="text-xs font-semibold text-slate-700 dark:text-slate-200 leading-tight tabular-nums">
              {esCompleto ? '9 / 9' : `${order.paso_actual} / ${PRODUCTION_ORDER_STEPS.length}`}
            </p>
        ...
        {/* SKU del producto + clave de estilo */}
