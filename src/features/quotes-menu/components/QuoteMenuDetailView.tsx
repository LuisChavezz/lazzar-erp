...
import { quoteMenuCards } from "../constants/quoteCardItems";

const EmbroideryOrderView = dynamic(
  () => import("@/src/features/embroidery-quotes/components/EmbroideryQuotesView"),
  { loading: () => <LoadingSkeleton /> }
);
const ProductionOrderView = dynamic(
  () => import("@/src/features/production-quotes/components/ProductionQuotesView"),
  { loading: () => <LoadingSkeleton /> }
);
const PurchaseOrderView = dynamic(
  () => import("@/src/features/purchase-quotes/components/PurchaseQuotesView"),
  { loading: () => <LoadingSkeleton /> }
);

interface QuoteMenuDetailViewProps {
  selectedView: string | null;
  onBack: () => void;
  onNavigate: (view: string) => void;
}

export function QuoteMenuDetailView({
  selectedView,
  onBack,
  onNavigate,
}: QuoteMenuDetailViewProps) {
  const renderHeader = () => (
  ...
  return (
    <div
      ...
      `}
    >
      {selectedView === "embroidery-quotes" && (
        <div className="flex flex-col gap-6 cur">
          {renderHeader()}
          <EmbroideryOrderView />
        </div>
      )}
      {selectedView === "production-quotes" && (
        <div className="flex flex-col gap-6">
          {renderHeader()}
          <ProductionOrderView />
        </div>
      )}
      {selectedView === "purchase-quotes" && (
        <div className="flex flex-col gap-6">
          {renderHeader()}
          <PurchaseOrderView />
        </div>
