...
const TrackingStatusBadge = ({ status }: { status: ShipmentTrackingItem["status"] }) => {
  const styles: Record<ShipmentTrackingItem["status"], string> = {
    ...
    Atrasado: "bg-amber-50 text-amber-600 dark:bg-amber-500/10 dark:text-amber-400",
  };
...
export const shipmentTrackingColumns: ColumnDef<ShipmentTrackingItem>[] = [
  {
  ...
  },
  {
    accessorKey: "status",
    header: "Estado",
    cell: ({ row }) => <TrackingStatusBadge status={row.getValue("status")} />,
  },
  {
