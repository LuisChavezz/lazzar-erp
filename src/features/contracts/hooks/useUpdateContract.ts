...
import { updateContract } from "../services/actions";
import { ContractCreate } from "../interfaces/contract.interface";
import { ContractFormValues } from "../schemas/contract.schema";

interface UpdateContractPayload extends ContractCreate {
  id: number;
}
...
export const useUpdateContract = (setError?: SetContractError) => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ id, ...values }: UpdateContractPayload) => updateContract(id, values),
    onSuccess: () => {
    ...
    onError: (error) => {
      ...
      toast.error(formError ?? "Error al actualizar el contrato");
    },
  });
};
