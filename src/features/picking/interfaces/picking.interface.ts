 ...
 * la UI. `almacen` (ORIGEN) tampoco es un campo del formulario: siempre viaja
 * como el id fijo del almacén "Producto Terminado"
 * (`PRODUCTO_TERMINADO_ALMACEN_ID` en `usePickingStep2Form`), confirmado
 * estable en todos los ambientes.
 *
 * `almacen_destino` SÍ es un campo del formulario (Paso 1) y viaja siempre
 * explícito. El backend acepta omitirlo —cae al `APARTADOS` de la empresa/
 * sucursal del pedido y solo lanza 400 si tampoco existe—, pero depender de
 * ese fallback dejaría el destino real fuera de la vista del usuario. Se
 * valida contra el pedido: misma empresa y sucursal, distinto del origen y con
 * `permite_entrada` en el catálogo.
 *
...
export interface CreatePickingPayload {
  pedido: number;
  operador: number;
  /** Almacén ORIGEN (id fijo — ver la nota de arriba). */
  almacen: number;
  /** Almacén DESTINO elegido en el Paso 1. */
