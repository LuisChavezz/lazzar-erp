...
import type { RequestFormContentProps } from "../interfaces/request-form.interface";
// ── Componentes hijos REUTILIZADOS de cotizaciones (no clonados) ────────────
// Los cinco diálogos y el buscador de cliente se importan tal cual. Son piezas
// autocontenidas cuyo contrato es `QuoteItem`, estructuralmente idéntico a
// `RequestItem`. Clonarlos multiplicaría por cinco la superficie a mantener sin
// ganar nada: la divergencia que se busca está en el CUERPO del formulario.
import { CustomerSearchDropdown } from "../../quotes/components/CustomerSearchDropdown";
import { AddProductDialog } from "../../quotes/components/AddProductDialog";
import { EditEmbroideryDialog } from "../../quotes/components/EditEmbroideryDialog";
import { EditReflectiveDialog } from "../../quotes/components/EditReflectiveDialog";
import { EditSizesDialog } from "../../quotes/components/EditSizesDialog";
...
/**
 * Cuerpo del formulario de solicitudes.
 *
 * CLON del JSX de `quotes/components/QuoteForm.tsx` (`QuoteFormContent`), con
 * una sola diferencia funcional: el selector "Tipo de Pedido" NO está
 * bloqueado (ver más abajo). Es una copia editable a propósito — solicitudes
 * va a divergir en campos y validaciones, y esa divergencia debe ocurrir aquí
 * sin tocar cotizaciones.
 *
 * Sus props las declara `RequestFormContentProps`, un contrato propio del
 * módulo; ya no es `ReturnType<typeof useQuoteForm>`.
 */
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Datos principales de la solicitud y configuración comercial.
              </p>
            </div>
          </div>
              ...
              <form.Field name="tipo_pedido">
                {(field) => (
                  <FormSelect
                    ...
                    // DIFERENCIA respecto a cotizaciones: aquí el select está
                    // habilitado. En cotizaciones lleva `disabled` fijo en el
                    // JSX y arranca en "PEDIDO DE VENTA"; en solicitudes el
                    // usuario puede elegir cualquier tipo del catálogo y el
                    // valor inicial es "MUESTRA" (ver `createEmptyValues` en
                    // `useRequestForm`).
                    name={field.name}
              ...
              {(field) => (
                <label className="inline-flex items-center gap-2 text-xs font-medium cursor-pointer text-slate-600 dark:text-slate-300">
                  <input
                    ...
                    onBlur={field.handleBlur}
                  />
                  Embarcar con otras solicitudes
                </label>
              )}
            </form.Field>
          </div>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              aria-label="Agregar producto a la solicitud"
            >
              Agregar Producto
            </button>
          </div>
        ...
        <div className="flex-1 overflow-auto -mx-6 px-6 pb-2 border-b border-slate-200 dark:border-slate-800">
          <table className="w-full min-w-300 border-collapse text-left">
            <caption className="sr-only">
              Tabla de partidas de la solicitud con acciones para editar o eliminar
            </caption>
