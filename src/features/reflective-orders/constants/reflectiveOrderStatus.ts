 ...
 * (indexado por el entero `estatus_reflejante`) y la ETIQUETA también cuando el
 * código está en 1-7. `estatus_reflejante_display` del backend solo entra como
 ...
 */
export const reflectiveStatusEntry = (
  estatus: number,
  display?: string | null,
): StatusBadgeConfigEntry => {
  const base = REFLECTIVE_ORDER_STATUS_CONFIG[String(estatus)] ?? NEUTRAL_STATUS_CFG;
