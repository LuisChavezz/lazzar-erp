import type { ReflectiveOnboardingOperador } from "../interfaces/reflective-order.interface";
...
/**
 * Nombre del operador que quedará asignado a la orden de reflejante.
 *
 * SIEMPRE es el usuario autenticado: `OrdenReflejanteService.save` hace
 ...
 *
 * Gemela de la de bordado, y duplicada a propósito: la firma depende de
 * `ReflectiveOnboardingOperador`, un tipo de ESTE módulo. Compartirla obligaría
 * a extraer un tipo común a `src/utils/` y a acoplar dos módulos que hoy solo
 * se parecen; la convención del proyecto es que cada feature declare sus
 * propios contratos aunque coincidan en forma (ver la nota de
 * `reflective-order.interface.ts`).
 */
