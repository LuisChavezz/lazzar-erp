 ...
 * (indexado por el entero `estatus_corte`) y la ETIQUETA también cuando el
 * código está en 1-7. `estatus_corte_display` del backend solo entra como
 ...
 */
export const corteMangaStatusEntry = (
  estatus: number,
  display?: string | null,
): StatusBadgeConfigEntry => {
  const base = CORTE_MANGA_ORDER_STATUS_CONFIG[String(estatus)] ?? NEUTRAL_STATUS_CFG;
