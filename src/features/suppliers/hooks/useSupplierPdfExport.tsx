...
const getColumnWeight = (column: DataTableVisibleColumn<Supplier>): number => {
  if (column.id === "proveedor") return 1.6;
  if (column.id === "razon_social") return 1.8;
  if (column.id === "contacto") return 1.6;
  return 1;
};

const createSuppliersPdfDocument = (
  renderer: Pick<
      ...
      <Page size="A4" orientation="landscape" style={styles.page}>
        ...
        <View style={styles.headerSection}>
          <View>
            ...
            <Text style={styles.title}>Catálogo de Proveedores</Text>
          </View>
        ...
        <View style={styles.table}>
          ...
          {suppliers.map((supplier, rowIndex) => (
            <View
              key={`${supplier.id}-${rowIndex}`}
              style={[
                styles.row,
                rowIndex % 2 !== 0 ? styles.rowOdd : {},
                rowIndex === suppliers.length - 1 ? styles.rowLast : {},
              ]}
            >
              {columns.map((column, colIndex) => (
                ...
                >
                  {getSupplierColumnText(supplier, column)}
                </Text>
              ))}
            </View>
          ))}
        ...
        <View style={styles.footer} fixed>
          <Text style={styles.footerText}>
            {suppliers.length} registro{suppliers.length !== 1 ? "s" : ""} — Generado con ERP Lazzar
          </Text>
          <Text
...
/**
 * Exporta el catálogo VISIBLE de proveedores a PDF. Mismo patrón que
 ...
 *
 * `getSuppliers` se invoca AL EXPORTAR (no al montar): son las filas filtradas y
...
export const useSupplierPdfExport = (
  ...
  const getSuppliersRef = useRef(getSuppliers);
  const columnsRef = useRef(columns);

  useEffect(() => {
    getSuppliersRef.current = getSuppliers;
  }, [getSuppliers]);
  ...
  const exportToPdf = useCallback(async () => {
    ...
    const pdfDocument = createSuppliersPdfDocument(
      {
  ...
  useEffect(() => {
    ...
    document.addEventListener("suppliers:exportPDF", handleExport);
    return () => {
      document.removeEventListener("suppliers:exportPDF", handleExport);
    };
  }, [exportToPdf]);
