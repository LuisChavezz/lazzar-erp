...
interface QuoteSetStatusSelectableRowProps {
  quote: Quote;
  isSelected: boolean;
  onToggle: (quoteId: number) => void;
}

export function QuoteSetStatusSelectableRow({
  quote,
  isSelected,
  onToggle,
}: QuoteSetStatusSelectableRowProps) {
  ...
  return (
    <button
      ...
      role="listitem"
      aria-label={`Seleccionar pedido ${quote.id}`}
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 truncate">
              {quote.cliente_nombre || quote.cliente_razon_social}
            </p>
          </div>
          <span
        ...
        <div className="flex items-center justify-between text-[10px] text-slate-400 mt-1">
          ...
          <span>{formatCurrency(Number(quote.gran_total) || 0)}</span>
        </div>
      </div>
    </button>
  );
