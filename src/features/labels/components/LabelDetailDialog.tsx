...
import { MOCK_LABEL_PRINTERS } from "../constants/labelPrinters";
...
const LabelPreview = ({ label }: { label: Label }) => (
  <div className="rounded-xl border-2 border-dashed border-slate-200 dark:border-white/15 bg-white dark:bg-zinc-800/40 px-5 py-4">
    <div className="flex items-start justify-between gap-4">
      <div className="min-w-0">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
          Color: <span className="font-medium">{label.color_nombre}</span>
          <span className="mx-2 text-slate-300 dark:text-slate-600">|</span>
          Talla: <span className="font-medium">{label.talla_nombre}</span>
        </p>
      </div>
    ...
    <div className="mt-4 flex items-end gap-[2px] h-14" aria-hidden="true">
      {barcodeBars(label.sku).map((width, index) => (
        <div
          key={index}
    ...
    <p className="mt-3 text-sm font-bold text-slate-800 dark:text-white">COD: {label.codigo}</p>
  </div>
);

interface LabelDetailDialogProps {
  /** La etiqueta ya cargada por el listado — sin fetch propio (el fixture de
   *  `useLabels` ya trae el registro completo, ZPL incluido). */
  label: Label;
  open: boolean;
 ...
 * `constants/labelPrinters.ts` (no una detección real ni un dato por
 ...
 * módulo.
 */
export function LabelDetailDialog({ label, open, onOpenChange }: LabelDetailDialogProps) {
  // Selección local de impresora — no es un dato de `label` (ver
  // `constants/labelPrinters.ts`), así que vive como estado propio del
  // diálogo y se reinicia a la primera opción cada vez que se abre (el
  // diálogo se desmonta al cerrarse, ver `LabelColumns.tsx`).
  const [selectedPrinterId, setSelectedPrinterId] = useState(MOCK_LABEL_PRINTERS[0].id);
  const selectedPrinter =
    MOCK_LABEL_PRINTERS.find((printer) => printer.id === selectedPrinterId) ??
    MOCK_LABEL_PRINTERS[0];
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-3 px-4 py-3 rounded-xl bg-slate-50 dark:bg-white/5 text-xs">
          ...
          <InfoField label="Variante">
            {label.color_nombre} / {label.talla_nombre}
          </InfoField>
          <InfoField label="Última impresión">
            <StatusBadge status={label.estado} config={LABEL_ESTADO_CONFIG} />
          </InfoField>
        </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-3 flex-1 min-w-0 text-xs">
                <FormSelect
                  ...
                  options={MOCK_LABEL_PRINTERS.map((printer) => ({
                    value: printer.id,
