...
const ActionsCell = ({
  ...
  row: Row<SatUnitCode>;
  onEdit: (code: SatUnitCode) => void;
}) => {
  const deleteSatUnitCode = useSatUnitCodeStore((state) => state.deleteSatUnitCode);

  const handleDelete = () => {
    deleteSatUnitCode(row.original.id_sat_unidad);
...
export const getColumns = (onEdit: (code: SatUnitCode) => void, isAdmin: boolean) => {
  const columns = [
