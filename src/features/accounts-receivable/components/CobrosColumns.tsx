...
import type { MockCobro } from "../interfaces/accounts-receivable.interface";

const METODO_CFG: Record<string, string> = {
  Transferencia: "bg-sky-50 text-sky-700 dark:bg-sky-500/10 dark:text-sky-400",
  ...
  Tarjeta: "bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400",
};
...
const columnHelper = createColumnHelper<MockCobro>();

export const cobrosColumns = [
  ...
  columnHelper.accessor("fecha_cobro", {
    header: "Fecha cobro",
    cell: (info) => (
  ...
  columnHelper.accessor("cxc_folio", {
    header: "CxC aplicada",
    cell: (info) => (
