...
/**
 * Lista de pedidos para Operaciones de Almacén — misma tabla que la Mesa de
 * Control (`GET /ventas/pedidos/`, hook `useOrders` compartido), pero de solo
 * lectura: desde almacén no se confirma la fecha del pedido.
 */
export function WmsOrderList() {
  const { orders, isLoading, isError, error } = useOrders();
  const queryClient = useQueryClient();
  const router = useRouter();
  const isRefetching = useIsFetching({ queryKey: ['orders'] }) > 0;

  // Detalle 360° del pedido en su ruta neutra; `?from=wms` hace que el "Volver"
  // regrese a esta lista.
  const handleViewDetail = (order: Order) => router.push(`/orders/${order.id}?from=wms`);
