...
const getColumnWeight = (column: DataTableVisibleColumn<PedidoListItem>): number => {
  ...
  if (column.id === "cliente") return 2;
  return 1;
};

const createProcurementOrdersPdfDocument = (
  renderer: Pick<
          ...
          {orders.map((order, rowIndex) => (
            ...
            >
              {columns.map((column, colIndex) => (
                ...
                >
                  {getProcurementOrderColumnText(order, column)}
                </Text>
              ))}
            </View>
          ))}
...
/**
 * Exporta el listado VISIBLE de pedidos (Compras/SCM) a PDF. Mismo patrón
 * que `usePurchaseOrderPdfExport`/`useQuotePdfExport`: `@react-pdf/renderer`
...
export const useProcurementOrderPdfExport = (
  ...
  const exportToPdf = useCallback(async () => {
    ...
    const pdfDocument = createProcurementOrdersPdfDocument(
      {
  ...
  useEffect(() => {
    ...
    document.addEventListener("procurement-orders:exportPDF", handleExport);
    return () => {
      document.removeEventListener("procurement-orders:exportPDF", handleExport);
    };
  }, [exportToPdf]);
