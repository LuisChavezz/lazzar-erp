...
**Server-side page guards are UX, not a security boundary.** The real access boundary is the backend on every mutation. Never write a server-side guard that queries the backend for access — it will always receive 401 (DRF authenticates before evaluating existence or permissions) and its only reachable branch will be "allow" (structural fail-open). Real access verification lives in client-side hooks, which are the only side with credentials. Reintroducing server-side *backend* verification only makes sense if frontend and backend share a registerable domain, and must be a conscious step of that migration. Reading the **NextAuth JWT** server-side is a different thing and is fine: its cookie lives on this app's own origin and carries `role` + `permissions`, so `getServerSession(authOptions)` is a local, fail-closed check — the same source `src/proxy.ts` reads with `getToken`. See `quotes/services/quoteEditAccess.server.ts`: it validates the id shape and the `E-CRM-COTIZACIONES` permission (both local, no network), and deliberately leaves existence/status/authorization of the record itself to the client.
...
Routes are grouped by business domain rather than kept flat: `system/`, `sales/`, `orders/`, `operations/`, `wms/`, `procurement/`, `manufacturing/`, `finance/`, `hr/`, plus `config/`. Each maps to a permission code (`R-CORE`, `R-CRM`, `R-MESACONTROL`, `R-WMS`, `R-COMPRAS`, `R-PRODUCCION`, `R-CONTABILIDAD`, `R-RH`, `R-CONFIGURACION`) — except `orders/`, the neutral pedido 360° detail route (`/orders/[id]`, reached from several modules via `?from=`), whose rule requires **any one** of a set of codes rather than a single module code, since it is legitimately reachable from Ventas, Mesa de Control, WMS, Compras and Producción. That is why a rule's `permission` is `string | string[]`: an array means "any one of these" (`hasAnyPermission`), and a lone string behaves exactly as before. Do not treat `/orders` as ungated — the backend filters this route's accounting fields by role, but not the related documents fetched from it, so the rule is the only access control. Group/sub-route definitions, sidebar labels, and module icons are centralized in `src/constants/appRoutes.ts`; the route-prefix → permission map and post-login redirect priority live in `src/constants/routePermissions.ts`. The sidebar (`getSidebarSectionsByPath`) is context-aware — it lists the top-level modules on `/` and `/config`, and the active module's sub-routes when inside one.
...
- **`DataTable.tsx`** — Full TanStack Table wrapper with sorting, pagination (10/20/50/100), global search, column visibility, drag-to-reorder columns, column resize, filter chips, and a refresh button. The `eslint-disable react-hooks/incompatible-library` comment is intentional. Notable props:
  - **Approved design is the default** ("panel mode"): `framed` (single frame around toolbar/table/pager; also the switch that sets the 48px min row height), `searchAlwaysExpanded` (fixed search input at the left of the toolbar) and `density="compact"` all default to `true`/`"compact"`. They remain escape hatches (`false` / `"comfortable"` restore the previous look) but no consumer uses them today, so don't pass them. There is **no `title` prop**: the page heading comes from the global `Header` (`HeaderTitle` → `getPageTitle`, per route) or from the view that mounts the table (`ConfigDetailView` renders one per catalog).
  - **Alignment goes through `meta.align`, never through column classes.** `meta.align` (`"left" | "center" | "right"`, typed in `src/types/tanstack-table.d.ts`) is applied by the `<th>`/`<td>`. Default is **left**. **Money and numeric amounts/quantities are right** (`meta: { align: "right" }`, keep `tabular-nums` on the cell so digits line up); "Acciones", status indicators and deliberately centered columns use `meta: { align: "center" }`. Don't put `text-center`/`text-right`/`w-full` on header or cell content (a `w-full` header wrapper pushes the sort arrow to the cell edge). A right-aligned header keeps its label flush right with the sort arrow to its left. A cell whose content is a flex container (e.g. the Acciones menu) must center itself with `justify-center`; `meta.align` only aligns the header with it. `meta.label` names the column in the column picker/exports when `header` is a component; `meta.hideOnMobile` hides it below `md`.
  - **`ColumnHeaderFilter.tsx`** is the Excel-style filter icon inside a header cell (status filters). It uses TanStack's native `columnFilters` state and needs the column to declare its own `filterFn`; it is separate from `filterConfig` (chips) and never appears in the chips row.
...
- **`DetailDialogPrimitives.tsx`** — presentational building blocks for detail pages and dialogs: `Section` (card with title and optional `action` prop), `InfoGrid` + `InfoField` (label/value grid), `LineItemsTable` (embedded table chrome with sticky `<thead>`), `EmptyLines`, and `textOrDash` (normalizes `null`/`""` to "—"). `LineItemsTable` provides only the chrome; each section declares its own columns as children — do not turn it into a config-driven table (that would reinvent `DataTable`). 28 consumers across the codebase.
  - **"No extra fetch" pattern:** when the list and retrieve endpoints share the same shape, the detail dialog/page receives the already-loaded row object by props and makes **no fetch of its own** (e.g. `ShippingDetailDialog`, `PackingColumns`, `CorteMangaOrderDetailDialog`). Document in the component that the shapes are identical; if they diverge, add a dedicated hook.
- **`MainDialog.tsx` / `ConfirmDialog.tsx`** — Radix Dialog wrappers; use these for all modals. One deliberate exception: a **command-palette surface** uses `@radix-ui/themes` `Dialog.Root`/`Dialog.Content` directly, because `MainDialog` forces a visible title and a red "Cerrar" footer and blocks outside-click close. The only one today is `src/features/search/` (`GlobalSearchPalette`, the global search opened from the header searchbar); follow it rather than inventing a third pattern.
